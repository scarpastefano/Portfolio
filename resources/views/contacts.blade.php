<x-main>
    ...
    <form action="{{ route('send') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid gap-6 mb-3 md:grid-cols-2">
            <div>
                <label for="name" class="block mb-2 text-sm font-medium text-white">Nome e cognome</label>
                <input type="text" id="name" name="name" class="outline-none border-0 border-gray-300 text-sm rounded-lg block w-full p-3 mb-6 bg-zinc-700 dark:border-gray-600 placeholder-gray-400 text-white" placeholder="Nome e cognome" required />
            </div>
            <div>
                <label for="email" class="block mb-2 text-sm font-medium text-white">Email</label>
                <input type="email" id="email" name="email" class="outline-none border-0 border-gray-300 text-sm rounded-lg block w-full p-3 mb-6 bg-zinc-700 dark:border-gray-600 placeholder-gray-400 text-white" placeholder="email" required />
            </div>
        </div>
